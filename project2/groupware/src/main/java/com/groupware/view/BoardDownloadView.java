...
public class BoardDownloadView implements View {
    ...
    public void render(Map<String, ?> model,
        ...
        resp.addHeader("Content-Disposition",
                "Attachment;filename=\"" +
                        new String(attach.getBoardUserFileName().getBytes("utf-8"), "ISO-8859-1") + "\"");

        //ServletContext : JSP의 application객체와 동일한 객체
        ServletContext application = req.getServletContext();
        String path = application.getRealPath("/board-attachments/" + attach.getBoardSavedFileName()); // 웹경로 --> 컴퓨터 경로

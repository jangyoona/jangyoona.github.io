$(function() {
    $("#chatbot-client").load("chatbot.html");
    // DOM 요소를 변수에 저장
    ...
    const handleChat = async () => {
        ...
        const response_data = await fetchData(`http://localhost:9999/chatbot/chat-text?userId=${userId}`, {
            method: "POST",
            cache: "no-cache",
    ...
    function fetchChatRooms() {
        fetchData(`http://localhost:9999/chatbot/chat-list?userId=${userId}`, {
            method: "GET",
            cache: "no-cache",
            headers: { "Content-Type": "application/json;charset=utf-8" }
        }).then(data => {
    ...
    function loadChatRooms() {
        chatRooms.forEach(room => {
            const roomElement = $(`
                <div class="chat-room" data-listid="${room.GPTTALKID}" id="chat-room-${room.GPTTALKID}">
                    <h5>Chat Room ${room.GPTTALKID}</h5>
        ...
        $('.delete-button').on('click', function(event) {
            ...
            fetchData(`http://localhost:9999/chatbot/chat-list-delete?gptTalkId=${talkId}`, {
                method: "GET",
                cache: "no-cache",
                headers: { "Content-Type": "application/json;charset=utf-8" }
            }).then(() => fetchChatRooms());
        ...
        $('.chat-room').on('click', function(event) {
            ...
            fetchData(`http://localhost:9999/chatbot/chat-history?gptTalkId=${talkId}&userId=${userId}`, {
                method: "GET",
                cache: "no-cache",
                headers: { "Content-Type": "application/json;charset=utf-8" }
            }).then(chat_list => {
    ...
    function sendAudioToServer() {
      ...
      fetch('http://localhost:9999/chatbot/audio-to-text', {
          method: 'POST',
          body: formData
      })
